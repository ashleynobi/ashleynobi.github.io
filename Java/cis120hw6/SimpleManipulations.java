...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        int w = pic.getWidth();
        int h = pic.getHeight();
        int ww = w + (2* borderWidth);
        int hh = h + (2* borderWidth);
       
      
        Pixel[][] src = pic.getBitmap();
        Pixel[][] tgt = new Pixel[ww][hh];
    ...
    public static PixelPicture scaleColors(
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                int sR = (int) Math.round(rfactor * r);
                int sG = (int) Math.round(gfactor * g);
                int sB = (int) Math.round(bfactor * b);
          
             
                bmp[x][y] = new Pixel(sR, sG, sB);
            }
        }
        return new PixelPicture(bmp);
    }
    ...
    public static PixelPicture alphaBlend(
        ...
        for (int x = 0; x < w; x++) {
            for (int y = 0; y < h; y++) {
                ...
                int gg = pp.getGreen();
                int bb = pp.getBlue();
                int wR = (int) weightedAverage(alpha, r, rr);
                int wG = (int) weightedAverage(alpha, g, gg);
                int wB = (int) weightedAverage(alpha, b, bb);
                bmp[x][y] = new Pixel(wR, wG, wB);
            }
        }
        return new PixelPicture(bmp);
